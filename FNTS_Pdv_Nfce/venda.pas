...
BEGIN
  if InternetStatus = tiOnline then begin
    ...
    lbStatus.Caption := 'ON-LINE';
    lbStatus.Font.Color := $00D5AA5B;
    imgON.Visible := True;
...
procedure TfrmVenda.lbStatusClick(Sender: TObject);
begin
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if frmModulo.ACBRNFCe.Configuracoes.Geral.FormaEmissao = frmtOffLine then begin
              vcontingencia := 'S';
              spNFCE_Insert.ParamByName('Contingencia').asstring := 'S';
              spNFCE_Insert.ParamByName('MotivoContigencia').asstring := MotivoContigencia;
            end else begin
              vcontingencia := 'N';
              spNFCE_Insert.ParamByName('Contingencia').asstring := 'N';
              spNFCE_Insert.ParamByName('MotivoContigencia').asstring := '';
            end;
...
procedure TfrmVenda.horaTimer(Sender: TObject);
begin
    DataLocal.caption := DateToStr(now) + ' / ' + TimeToStr(time);
   // HoraLocal.caption := TimeToStr(time);
end;

procedure TfrmVenda.imgOffClick(Sender: TObject);
begin
...
procedure TfrmVenda.tmr1Timer(Sender: TObject);
begin
